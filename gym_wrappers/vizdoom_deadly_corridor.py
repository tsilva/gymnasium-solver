...
class VizDoomDeadlyCorridorEnv(gym.Env):
    """Minimal Gymnasium wrapper around ViZDoom Deadly Corridor scenario.

    This environment expects ViZDoom to be installed (pip install vizdoom).

    Parameters (via env_kwargs):
        config_path: Optional[str] - Path to deadly_corridor.cfg. If not provided,
    ...
    def __init__(
        ...
        **_: Dict[str, Any],
        ...
        try:
            ...
            raise ImportError(
                "vizdoom package is required for VizDoomDeadlyCorridorEnv. "
        ...
        if cfg_path is None:
            raise FileNotFoundError(
                "Could not locate deadly_corridor.cfg. Set env_kwargs.config_path "
        ...
        # We construct single-button and a useful combo Forward+Attack
    ...
    def _resolve_config_path(self, config_path: Optional[str]) -> Optional[Path]:
        ...
        if env_dir:
            p = Path(env_dir) / "deadly_corridor.cfg"
            if p.is_file():
                return p
        ...
        try:
            ...
            p = scenarios_dir / "deadly_corridor.cfg"
            if p.is_file():
                return p
        except Exception:
            pass
