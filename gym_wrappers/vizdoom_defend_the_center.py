...
class VizDoomDefendTheCenterEnv(gym.Env):
    """Minimal Gymnasium wrapper around ViZDoom Defend The Center scenario.

    This environment expects ViZDoom to be installed (pip install vizdoom).

    Parameters (via env_kwargs):
        config_path: Optional[str] - Path to defend_the_center.cfg. If not provided,
    ...
    def __init__(
        ...
        **_: Dict[str, Any],
        ...
        try:
            ...
            raise ImportError(
                "vizdoom package is required for VizDoomDefendTheCenterEnv. "
        ...
        if cfg_path is None:
            raise FileNotFoundError(
                "Could not locate defend_the_center.cfg. Set env_kwargs.config_path "
    ...
    def _resolve_config_path(self, config_path: Optional[str]) -> Optional[Path]:
        ...
        if env_dir:
            p = Path(env_dir) / "defend_the_center.cfg"
            if p.is_file():
                return p
        ...
        try:
            ...
            p = scenarios_dir / "defend_the_center.cfg"
            if p.is_file():
                return p
        except Exception:
            pass
