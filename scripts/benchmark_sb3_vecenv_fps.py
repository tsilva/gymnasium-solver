"""Compare ALE throughput via Stable-Baselines3 vector environments.

Matches the CLI of `benchmark_alepy_fps.py` and adds `--subproc` to switch
between `DummyVecEnv` (default) and `SubprocVecEnv`.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark ALE FPS via Stable-Baselines3 Dummy/Subproc VecEnvs."
    )
...
def main() -> None:
    ...
    print("Env id:", env_id)
    backend = "SubprocVecEnv" if args.subproc else "DummyVecEnv"
