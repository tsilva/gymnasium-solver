...
def _make_logger():
    buf = io.StringIO()
    recorder = MetricsRecorder(step_key="train/cnt/total_timesteps")
...
def test_print_logger_preserves_sticky_values_between_calls():
    ...
    second = {
        "train/cnt/total_timesteps": 256.0,
    }
    ...
    assert "roll/ep_rew/mean" in out
...
def test_print_logger_trigger_alert_highlight_persists_and_clears():
    ...
    logger.metrics_monitor.active_alerts = {
        "train/roll/ep_rew/mean": ["training reward below threshold"],
    }

    # First payload includes an alert, which should activate yellow row highlight
    first = {
        "train/roll/ep_rew/mean": 5.0,
    }
    logger.log_metrics(first)
    ...
    assert "train/roll/ep_rew/mean" in logger.metrics_monitor.active_alerts
